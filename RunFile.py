...
def Run(strategy, params, file_path, divide_to_months):
    ...
    if divide_to_months:
        ...
        for index, v in enumerate(divided_dates):
            from_d = datetime.datetime.fromtimestamp(v[0].value / 1e9).strftime('%Y-%m-%d')
            to_d = datetime.datetime.fromtimestamp(v[1].value / 1e9).strftime('%Y-%m-%d')
            month_title = from_d.split('-')[0] + '/' + from_d.split('-')[1] + '-' + \
        ...
        for res in results:
            for strat in res:
                ...
                if strat.strategycls.__name__ == "BuyAndHold":
                    ...
                    results_data_rows['Total_MaxDD(%)_Strategy'].append(
                        strat.analyzers.drowdown.get_analysis()['max']['drawdown'])
                    results_data_rows['Total_MaxDD(Time)_Asset'].append(max_dd_asset_time)
                    results_data_rows['Total_MaxDD(Time)_Strategy'].append(
                        strat.analyzers.timedrowdown.get_analysis()['maxdrawdownperiod'])
                    if len(results) == 1:
                        df = pd.DataFrame([(x, y, z) for ((x, y), z) in
                                           zip(list(strat.analyzers.logreturn.get_analysis().items()),
                                               sum_return_asset)])
                    ...
                    for index, item in enumerate(divided_dates):
                        for i in list(strat.analyzers.logreturn.get_analysis().items()):
                            if i[0] >= item[0] and i[0] < item[1]:
                                if strat.strategycls.__name__ == "BuyAndHold":
                                    month_returns_asset[index]["Month" + str(index + 1)].append(i[1])
                                else:
                                    month_returns_strategy[index]["Month" + str(index + 1)].append(i[1])
                        if strat.strategycls.__name__ != "BuyAndHold":
                            results_data_rows['Return_Asset_' + str(index + 1)].append(
                            ...
                            try:
                                results_data_rows['STD_' + str(index + 1)].append(
                                    (average(month_returns_strategy[index]["Month" + str(index + 1)]) -
                                     average(month_returns_asset[index]["Month" + str(index + 1)])) /
